...
const ValidationBase = styled.div(
  ({ theme }) => css`
    .valid {
...
const AddWalletButton = styled(InvisibleButton)(
  ({ theme }) => css`
    .inner {
...
export const MobileAddButton = styled(InlineButton)(
  ({ theme }) => css`
    width: 50px;
    height: 50px;
    max-width: 50px;
...
const Wallets = () => {
  ...
  const connectPhantomWallet = async () => {
    try {
      ...
      await window.solana.on("connect", () =>
        addWallet("phantom", window.solana.publicKey.toString())
      );
  ...
  const connectSolflareWallet = async () => {
    try {
      ...
      window.solflare.on("connect", () =>
        addWallet("solflare", window.solana.publicKey.toString())
      );
    } catch {}
  };
  ...
  const connectSlopeWallet = async () => {
    try {
      ...
      window.slope.on("Connect", () =>
        addWallet("slope", window.solana.publicKey.toString())
      );
    } catch {}
  };
              ...
              {wallets.length > 0 ? (
                <div className="d-flex py-4 flex-column">
                  {wallets.map((wallet, index) => (
                    <WalletCard
                      className="p-4 mb-3 d-flex justify-content-between w-100 align-items-center overflow-auto"
                      key={index + 1}
                    >
                      <div className="d-flex">
                        {(wallet.type === "local" && (
                          <i className="material-icons walletIcon">wallet</i>
                        )) ||
                          (wallet.type === "phantom" && (
                            <CustomWalletIcon src={PhantomIcon} alt="Phantom" />
                          )) ||
                          (wallet.type === "slope" && (
                            <CustomWalletIcon src={SlopeIcon} alt="Slope" />
                          )) ||
                      ...
                      <div className="d-flex">
                        <InvisibleButton
                          onClick={() => toggleEditWallet(wallet)}
                        >
                          <i className="material-icons edit">edit</i>
                        </InvisibleButton>
                        <InvisibleButton
                          onClick={() => toggleDeleteWallet(wallet, index + 1)}
                        >
            ...
            {modalMode === "local" && (
              <ModalBody>
                ...
                <FormInput
                  ...
                  className={`
										w-100 ${(walletAddress.length > 44 || walletAddress.length < 32) && "invalid"}
									`}
                />
                <PrimaryButton
